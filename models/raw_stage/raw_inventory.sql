...
FROM {{ source('raw_data', 'raw_partsupp') }} AS a
LEFT JOIN {{ source('raw_data', 'raw_supplier') }} AS b
    ON a.PS_SUPPKEY = b.S_SUPPKEY
LEFT JOIN {{ source('raw_data', 'raw_part') }} AS c
...
-- order is not allowed in teradata
